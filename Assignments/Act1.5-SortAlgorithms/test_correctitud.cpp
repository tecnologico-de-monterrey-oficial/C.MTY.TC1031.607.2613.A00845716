// Prueba interna de correctitud (NO es parte de la entrega principal).
// Verifica que los 7 algoritmos ordenen bien en varios casos borde.
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
bool probar(int id, vector<T> v) {
    vector<T> esperado = v;
    sort(esperado.begin(), esperado.end()); // referencia (solo para test)
    aplicarOrden(id, v);
    return v == esperado;
}
